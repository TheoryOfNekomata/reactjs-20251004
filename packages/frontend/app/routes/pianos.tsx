...
export default function IndexPage() {
	...
	const {data: pianoData, isLoading: isLoadingPianoData} = useQuery({
		...
		queryFn: () => pianoService.queryPianos({
						...
						<div className="my-8">
							<form onSubmit={paginate}>
								...
								<div className="flex gap-4 justify-center">
									...
									<div className="w-32">
										<Button type="submit" name="action" value="next" disabled={typeof pianoData === 'undefined' || Number(searchParams.get('p') ?? '1') > Math.floor(pianoData.count / Number(searchParams.get('c') ?? '12'))}>
											Next
										</Button>
									</div>
								</div>
